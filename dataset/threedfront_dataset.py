...
from psutil import FreeMemLinux
from helpers_util import normalize_box_params
...
class ThreedFrontDatasetSceneGraph(data.Dataset):
    def __init__(self, root, root_3dfront='', split='train', shuffle_objs=False, pass_scan_id=False, use_SDF=False,
        ...
        ## bycc ++++++
        # self.instruction = "Represent the Science document for summarization:"
        # self.model_instructor = INSTRUCTOR('hkunlp/instructor-base').cuda()
        ## bycc ======
        ...
        self.relationships_dict = dict(zip(self.relationships,range(1,len(self.relationships)+1))) # 'left': 1
        self.relationships_dict_r = dict(zip(self.relationships_dict.values(), self.relationships_dict.keys())) # 1: 'left'
        ...
        self.relationship_json, self.objs_json, self.tight_boxes_json = self.read_relationship_json(self.rel_json_file, self.box_json_file)
        ...
        if self.root_3dfront == '':
            ...
            ## bycc ++++++
            # self.root_3dfront = os.path.join(self.root, 'visualization2')
            ## bycc ======
            if not os.path.exists(self.root_3dfront):
                os.makedirs(self.root_3dfront)

        self.mapping_full2simple = json.load(open(os.path.join(self.root, "mapping.json"), "r")) # 'armchair': 'chair'
        ...
        self.point_classes_idx = points_classes_idx + [0] # last element in the list is 0
    ...
    def __getitem__(self, index):
        ...
        for key in keys:
            ...
            scene_instance_class = instance2label[key] # 'cabinet'
            if not self.large:
                scene_class_id_grained = self.fine_grained_classes[scene_instance_class] # 3
                scene_instance_class = self.mapping_full2simple[scene_instance_class] # 'cabinet'
                scene_class_id = self.classes[scene_instance_class] # 3
            ...
            if self.use_SDF:
                if self.tight_boxes_json[scan_id][key]["model_path"] is None:
                    ...
                    sdf = torch.Tensor(obj_sdf).view(1, self.sdf_res, self.sdf_res, self.sdf_res) # (262144, 1) -> (1, 64, 64, 64)
        ...
        if self.with_CLIP:
            ...
            if os.path.exists(self.clip_feats_path):
                ...
                clip_feats_ins = clip_feats_dic['instance_feats'] # (num_instance+1, 512)
                # ## bycc ++++++
                # clip_feats_rel = clip_feats_dic['rel_feats'] # 'wardrobe left table': 512
                # sentences = ', '.join(clip_feats_rel.keys())
                # llm_embed = self.model_instructor.encode([[self.instruction,sentences]]) # (1, 768)
                ...
                # ## bycc ======
                clip_feats_order = np.asarray(clip_feats_dic['instance_order']) # (num_instance)
                ordered_feats = []
        ...
        triples = []
        words = []
        rel_json = self.relationship_json[scan_id] # [subject_id, object_id, predicate_id, 'left']
                ...
                if 'rel_feats' in batch[i][key]:
                    if 'changed_id' in batch[i][key]:
                        ...
                        if self.with_CLIP:
                            ...
                            ## bycc ++++++
                            # batch[i][key]['rel_feats'][idx] = torch.from_numpy(np.concatenate((np.squeeze(rel), np.squeeze(self.model_instructor.encode([[self.instruction, batch[i][key]['words'][idx]]])))))
                            ## bycc ======
