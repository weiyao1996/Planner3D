...
sys.path.append('/home/weiy1/code/commonscenes/dataset/')
sys.path.append('/home/weiy1/code/commonscenes/model/')
sys.path.append('/home/weiy1/code/commonscenes/helpers/')
# from threedfront_dataset import ThreedFrontDatasetSceneGraph
from threedfront_dataset2 import ThreedFrontDatasetSceneGraph
from VAE2 import VAE
from VAE3 import SiamDiff
from discriminators import BoxDiscriminator, ShapeAuxillary
from losses import bce_loss
from helpers_util import bool_flag, _CustomDataParallel, batch_torch_denormalize_box_params
from losses import calculate_model_losses
...
parser.add_argument('--dataset', required=False, type=str, default="/home/weiy1/code/commonscenes/Data", help="dataset path")
parser.add_argument('--logf', default='logs', help='folder to save tensorboard logs')
parser.add_argument('--exp', default='/home/weiy1/code/commonscenes/experiments/g2sv2/all_final', help='experiment name')
parser.add_argument('--room_type', default='all', help='room type [bedroom, livingroom, diningroom, library, all]')
...
parser.add_argument('--with_SDF', type=bool_flag, default=True)  # TODO
...
parser.add_argument('--weight_D_box', default=0, type=float, help="Box Discriminator") # 0.1
parser.add_argument('--with_changes', default=False, type=bool_flag) # True
parser.add_argument('--loadmodel', default=True, type=bool_flag)
...
parser.add_argument('--diff_yaml', default='/home/weiy1/code/commonscenes/config/v2_full.yaml', type=str, help='config of the diffusion network [cross_attn/concat]')
parser.add_argument('--diff_yaml2', default='/home/weiy1/code/commonscenes/config/v3_full.yaml', type=str, help='config of the diffusion network [cross_attn/concat]')
...
def parse_data(data):
    ...
    try:
        encoded_enc_f[enc_scene_nodes] = torch.zeros([torch.sum(enc_scene_nodes), encoded_enc_f.shape[1]]).float().cuda()
        encoded_dec_f[dec_scene_nodes] = torch.zeros([torch.sum(dec_scene_nodes), encoded_dec_f.shape[1]]).float().cuda()
    except:
...
def train():
    ...
    normalized_file = os.path.join(args.dataset, 'boxes_centered_stats_{}_trainval.txt').format(args.room_type)
    ...
    model_cc = SiamDiff(root=args.dataset, type=args.network_type, diff_opt2=args.diff_yaml2, vocab=dataset.vocab,
                replace_latent=args.replace_latent, with_changes=args.with_changes, residual=args.residual,
                gconv_pooling=args.pooling, with_angles=args.with_angles, num_box_params=args.num_box_params,
                deepsdf=args.with_feats, clip=args.with_CLIP, with_E2=args.with_E2)
    ...
    for epoch in range(start_epoch, args.nepoch):
        ...
        for i, data in enumerate(dataloader, 0):
            ...
            # model = model.train()
            model = model.eval()
            model.compute_statistics(exp=args.exp, epoch=args.loadepoch, stats_dataloader=dataloader, force=False)
            ...
            with torch.no_grad():
                un_rel_feat, rel_feat, boxes_pred, boxes_pred_uc, angles_pred, orig_gt_d3, orig_gt_angles = model.sample_box_and_shape(dec_objs, dec_triples, dec_sdfs, encoded_dec_text_feat, encoded_dec_rel_feat, dec_boxes, dec_angles, attributes=None, gen_shape=True)
            ...
            boxes_pred_den_uc = batch_torch_denormalize_box_params(boxes_pred_uc,file=normalized_file)
            c_layout = torch.cat([boxes_pred_den, angles_pred], dim=1) # [n, 7]
            uc_layout = torch.cat([boxes_pred_den_uc, angles_pred], dim=1) # [n, 7]
            data_b0 = torch.cat([orig_gt_d3, orig_gt_angles.unsqueeze(1)], dim=1) # [n, 7]

            model_cc = model_cc.train()          
            model_cc.forward_mani(dec_objs_to_scene, dec_objs, dec_objs_grained, c_layout, uc_layout, data_b0)
            loss = model_cc.vae_v3.Diffcc.loss_df
            ...
            # # train E_c and D_l via KL loss and layout reconstruction loss
