...
# from helpers_util import bool_flag, batch_torch_denormalize_box_params, sample_points
...
sys.path.append('/home/weiy1/code/commonscenes/extension/')
from VAE import VAE
...
from helpers_util import bool_flag, batch_torch_denormalize_box_params, sample_points
from metrics_3dfront import validate_constrains, validate_constrains_changes, estimate_angular_std
from visualize_scene import render, render_v2_full, render_v2_box, render_v1_full
...
parser.add_argument('--num_samples', type=int, default=10, help='for diversity')
...
parser.add_argument('--exp', default='/home/weiy1/code/commonscenes/experiments/g2sv2/all_demo', help='experiment name') # create folder "checkpoint" and copy
# parser.add_argument('--exp', default='/home/weiy1/code/commonscenes/experiments/g2sv2/all_oursv1_wo_llm_w_iou', help='experiment name') # create folder "checkpoint" and copy
parser.add_argument('--epoch', type=str, default='180', help='saved epoch')
...
parser.add_argument('--gen_shape', default=True, type=bool_flag, help='infer diffusion')
...
parser.add_argument('--no_stool', default=True, type=bool_flag)
...
def validate_constrains_loop_w_changes(modelArgs, testdataloader, model, normalized_file=None, with_diversity=True, with_angles=False, num_samples=3, cat2objs=None, datasize='large', gen_shape=False):
    ...
    bbox_file = "/home/weiy1/code/commonscenes/Data/cat_jid_trainval.json" if datasize == 'large' else "/home/weiy1/code/commonscenes/Data/cat_jid_trainval_small.json"
        ...
        if args.visualize:
            ...
            if model.type_ == 'v1_box' or model.type_ == 'v2_box':
                render_v2_box(model.type_, data['scan_id'], dec_objs.detach().cpu().numpy(), boxes_pred_den, angles_pred, datasize=datasize, classes=classes, render_type='retrieval',
                       classed_idx=dec_objs, store_img=True, render_boxes=False, visual=True, demo=False, no_stool = args.no_stool, without_lamp=False) # True
            elif model.type_ == 'v1_full':
                render_v1_full(model.type_, data['scan_id'], dec_objs.detach().cpu().numpy(), boxes_pred_den, angles_pred, datasize=datasize, classes=classes, render_type='v1', classed_idx=dec_objs,
                    shapes_pred=shapes_pred, store_img=True, render_boxes=False, visual=False, demo=False, no_stool = args.no_stool, without_lamp=False) # True
