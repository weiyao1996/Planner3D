...
from graph import GraphTripleConvNet, GraphTripleConvNet2, _init_weights, make_mlp
from sdfusion_difflayo_model3 import Diffusion4Layout
...
from helpers_util import bool_flag, _CustomDataParallel
from lr_scheduler import *
from fvcore.common.param_scheduler import MultiStepParamScheduler


class Sg2ScVAEModel2024(nn.Module):
    """
    ...
    """
    def __init__(self, vocab, diff_opt2, diffusion_bs=8, embedding_dim=128, batch_size=32,
                 train_3d=True,
                 decoder_cat=False,
                 num_box_params=6,
                 ...
                 use_angles=False,
                 clip=True):
        super(Sg2ScVAEModel2024, self).__init__()
        gconv_dim = embedding_dim # 64
        gconv_hidden_dim = gconv_dim * 4 # 256
        box_embedding_dim = int(embedding_dim)
        if use_angles:
            angle_embedding_dim = int(embedding_dim / 4) # 16
            box_embedding_dim = int(embedding_dim * 3 / 4) # 48
            Nangle = 24
        obj_embedding_dim = embedding_dim # 64
        self.dist_before = distribution_before # True
        self.replace_all_latent = replace_latent # True
        self.batch_size = batch_size # 32
        self.embedding_dim = embedding_dim # 64
        self.train_3d = train_3d # True
        self.decoder_cat = decoder_cat # True
        self.vocab = vocab
        self.vec_noise_dim = vec_noise_dim
        self.use_AE = use_AE # False
        self.use_angles = use_angles # True
        # self.lr = lr_full
        self.clip = clip # True
        add_dim = 0
        if clip:
        ...
        ## bycc ++++++
        self.mean_var = make_mlp([embedding_dim * 2 + add_dim + 768, gconv_hidden_dim, embedding_dim * 2],
                                     batch_norm=mlp_normalization)
        if self.use_angles:
            self.angle_mean_var = make_mlp([embedding_dim * 2 + add_dim + 768, gconv_hidden_dim, embedding_dim * 2],
                                           batch_norm=mlp_normalization)
        ## bycc ======
        
        self.diffusion_bs = diffusion_bs
        self.use_E2 = use_E2
        self.diff_cfg2 = OmegaConf.load(diff_opt2)
        self.diffusion_bs = diffusion_bs if self.diff_cfg2.hyper.batch_size is None else self.diff_cfg2.hyper.batch_size
        self.Diffcc = Diffusion4Layout(self.diff_cfg2)

        # visualizer
        self.visualizer = Visualizer(self.diff_cfg2)
        if get_rank() == 0:
        ...
        gconv_kwargs_ec_box = {
            ...
            ## bycc ++++++
            'input_dim_obj': gconv_dim * 2 + add_dim + 768,
            'input_dim_pred': gconv_dim * 2 + add_dim + 768,
            ## bycc ======
        ...
        gconv_kwargs_manipulation = {
            ...
            ## bycc ++++++
            'input_dim_obj': embedding_dim * 3 + add_dim + 768,
            'input_dim_pred': embedding_dim * 3 + add_dim + 768,
            ## bycc ======
            'hidden_dim': gconv_hidden_dim,
        ...
        ## bycc ++++++
        net_layers = [gconv_dim * 2 + add_dim + 768, gconv_hidden_dim, num_box_params]
        ## bycc ======
        ...
        ## bycc ++++++
        net_rel_layers = [gconv_dim * 2 + add_dim + 768, 960, 1280]
        ## bycc ======
        if self.Diffcc.df.conditioning_key == 'concat':
        ...
        if self.use_angles:
            ...
            ## bycc ++++++
            angle_net_layers = [gconv_dim * 2 + add_dim + 768, gconv_hidden_dim, Nangle]
            ## bycc ======
    ...
    #     # [O], [T,3], [O,6], None, [O,512], [T,512], [O]
    #     O, T = objs.size(0), triples.size(0)
    #     s, p, o = triples.chunk(3, dim=1)  # All have shape (T, 1)
    #     s, p, o = [x.squeeze(1) for x in [s, p, o]]  # Now have shape (T,)
    #     edges = torch.stack([s, o], dim=1)  # Shape is (T, 2)

    #     obj_vecs = self.obj_embeddings_ec(objs) # [O,64]
    #     pred_vecs = self.pred_embeddings_ec(p) # [T,128]
    #     d3_vecs = self.d3_embeddings(boxes_gt) # [O,48]
    #     if self.clip:
    #         obj_vecs_ = torch.cat([enc_text_feat, obj_vecs], dim=1) #512+64
    #         pred_vecs_ = torch.cat([enc_rel_feat, pred_vecs], dim=1) #512+128
    #     else:
    #         obj_vecs_, pred_vecs_ = obj_vecs, pred_vecs
    #     if self.use_angles:
    #         angle_vecs = self.angle_embeddings(angles_gt) #[O,16]
    #         obj_vecs_ = torch.cat([obj_vecs_, d3_vecs, angle_vecs], dim=1) #640=(512+64)+48+16
    #     else:
    ...
    #     return mu, logvar
    # [O,64], [O], [T,3], [O,512], [T,512], None, False
    # def encoder_2(self, z, objs, triples, dec_text_feat, dec_rel_feat, attributes, manipulate=False):
    #     s, p, o = triples.chunk(3, dim=1)  # All have shape (T, 1)
    #     s, p, o = [x.squeeze(1) for x in [s, p, o]]  # Now have shape (T,)
    #     edges = torch.stack([s, o], dim=1)  # Shape is (T, 2)

    #     obj_vecs = self.obj_embeddings_dc(objs) # [O,64]
    #     pred_vecs = self.pred_embeddings_dc(p) # [T,128]
    #     if self.clip:
    #         obj_vecs_ = torch.cat([dec_text_feat, obj_vecs], dim=1) #512+64
    #         pred_vecs_ = torch.cat([dec_rel_feat, pred_vecs], dim=1) #512+128
    #     else:
    #         obj_vecs_, pred_vecs_=obj_vecs,pred_vecs
    #     # concatenate noise first
    #     rel_vecs_ = torch.cat([obj_vecs_, z], dim=1) #(512+64)+64
    #     rel_vecs_2 = None
    #     if self.use_E2:
    ...
    #         ## bycc ++++++++
    #         d3_pred = self.d3_net(rel_vecs_2) #[O, 6]
    #         angles_pred = F.log_softmax(self.angle_net(rel_vecs_2), dim=1) #[O, 24]
    #         rel_vecs_2_ = self.rel_mlp(rel_vecs_2)
    #         rel_vecs_2_ = torch.unsqueeze(rel_vecs_2_, dim=1)
    #         ## bycc ========
    #     d3_pred_uc = self.d3_net(rel_vecs_)
    ...
    #     ## bycc ++++++++
    #     return rel_vecs_, rel_vecs_2_, d3_pred, angles_pred, d3_pred_uc
    #     ## bycc ========
    ...
    def decoder(self, z, objs, triples, dec_text_feat, dec_rel_feat, attributes, manipulate=False):
        ...
        # concatenate noise first
        if self.decoder_cat:
            obj_vecs_ = torch.cat([obj_vecs_, z], dim=1) #(512+64)+64
        ...
        d3_pred = self.d3_net(obj_vecs_) #[O, 6]
        if self.use_angles:
            angles_pred = F.log_softmax(self.angle_net(obj_vecs_), dim=1) #[O, 24]
    ...
    # # [O], [O], [O], [O, 1, 64, 64, 64], [O, 1, 1280], [O, 1, 1280], False
    ...
    def select_boxes(self, dec_objs_to_scene, dec_objs, dec_objs_grained, c_layout, uc_layout, data_b0, random=False):
        dec_objs_to_scene = dec_objs_to_scene.detach().cpu().numpy()
        batch_size = np.max(dec_objs_to_scene) + 1
        sdf_selected = []
        uc_rel_selected = []
        ...
        num_obj = int(np.ceil(self.diffusion_bs / batch_size))
        for i in range(batch_size):
            sdf_candidates = data_b0[np.where(dec_objs_to_scene == i)[0]]
            obj_cat = dec_objs[np.where(dec_objs_to_scene == i)[0]]
            obj_cat_grained = dec_objs_grained[np.where(dec_objs_to_scene == i)[0]]
            uc_rel = uc_layout[np.where(dec_objs_to_scene == i)[0]]
            c_rel = c_layout[np.where(dec_objs_to_scene == i)[0]]
            length = obj_cat.size(0)
            zeros_tensor = torch.zeros_like(sdf_candidates[0])
            mask = torch.ne(sdf_candidates, zeros_tensor)
            # mask = torch.any(mask, dim=1)
    ...
    def forward(self, dec_objs_to_scene, dec_objs, dec_objs_grained, c_layout, uc_layout, data_b0):
        obj_selected, diff_dict = self.select_boxes(dec_objs_to_scene, dec_objs, dec_objs_grained, c_layout, uc_layout, data_b0, random=False)
        self.Diffcc.set_input(diff_dict)
        self.Diffcc.set_requires_grad([self.Diffcc.df], requires_grad=True)
        self.Diffcc.forward()

        # mu, logvar = self.encoder(enc_objs, enc_triples, enc, attributes, enc_text_feat, enc_rel_feat, enc_angles) # E_c
        # if self.use_AE:
        #     z = mu
        # else:
        #     # reparameterization
        ...
        ## bycc ++++++++
        # uc_rel_feat, c_rel_feat, dec_man_enc_pred, angles_pred = self.encoder_2(z, dec_objs, dec_triples, dec_text_feat, dec_rel_feat, attributes)
        # ## bycc ========
        # if c_rel_feat == None:
        ...
        # self.Diff.set_requires_grad([self.Diffcc.df], requires_grad=True)
        # self.Diff.forward()
        ...
        # # [O,64], [O,64], [keep,6], [keep], [keep,1,64,64,64], [keep,6], [keep,24], [O',6], [O',24], ?, [O',1]
    ...
    def sample(self, mean_est, cov_est, dec_objs, dec_triplets, dec_sdfs, encoded_dec_text_feat, encoded_dec_rel_feat, dec_boxes, dec_angles, attributes=None, gen_shape=False):
        with torch.no_grad():
            ...
            if gen_shape:
                ...
                ## bycc ++++++++
                un_rel_feat, rel_feat, dec_man_enc_pred, angles_pred, dec_man_enc_pred_uc = self.encoder_2(z, dec_objs, dec_triplets, encoded_dec_text_feat, encoded_dec_rel_feat, attributes)
                ## bycc ========
                ## bycc --------
                # sdf_candidates = dec_sdfs
                # length = dec_objs.size(0)
                ...
                orig_gt_d3 = []
                for i in range(len(dec_man_enc_pred)):
                    orig_gt_d3.append(dec_boxes[i:i+1])
                    orig_gt_angles.append(dec_angles[i:i+1])
                orig_gt_d3 = torch.cat(orig_gt_d3, dim=0)
                orig_gt_angles = torch.cat(orig_gt_angles, dim=0)
            ...
            ## bycc ++++++++
            return dec_man_enc_pred, dec_man_enc_pred_uc, angles_pred, orig_gt_d3, orig_gt_angles
            ## bycc ========
    ...
    def optimizer_ini(self):
        ...
        # diffusion
        df_params = self.Diffcc.trainable_params
    ...
    def state_dict(self, epoch, counter):
        state_dict_1 = super(Sg2ScVAEModel2024, self).state_dict()
        state_dict_2 = {
            'epoch': epoch,
            'counter': counter,
            # 'vqvae': self.Diff.vqvae_module.state_dict(),
            # 'cond_model': self.cond_model_module.state_dict(),
            'df': self.Diffcc.df_module.state_dict(),
