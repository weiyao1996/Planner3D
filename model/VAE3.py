...
from VAEGAN_V1BOX import Sg2ScVAEModel as v1_box
from VAEGAN_V1FULL import Sg2ScVAEModel as v1_full
from VAEGAN_V2BOX import Sg2ScVAEModel as v2_box
# from VAEGAN_V2FULL2 import Sg2ScVAEModel as v2_full
from VAEGAN_V3FULL3 import Sg2ScVAEModel2024 as v3_full


class SiamDiff(nn.Module):

    def __init__(self, root="../GT",type='v1_box', diff_opt2 = '../config/v2_full.yaml', vocab=None, replace_latent=False, with_changes=True, distribution_before=True,
        ...
        self.diff_opt2 = diff_opt2
        assert distribution_before is not None and replace_latent is not None and with_changes is not None
        self.vae_v3 = v3_full(vocab, self.diff_opt2, diffusion_bs=16, embedding_dim=64, decoder_cat=True, mlp_normalization="batch",
        ...
        self.vae_v3.optimizer_ini()
        self.counter = 0

    def set_cuda(self):
        self.vae_v3.set_cuda()

    def forward_mani(self, dec_objs_to_scene, dec_objs, dec_objs_grained, c_layout, uc_layout, data_b0):
        c_layout = c_layout.unsqueeze(1)
        uc_layout = uc_layout.unsqueeze(1)
        # angle = data_b0[:, -1]
        self.vae_v3.forward(dec_objs_to_scene, dec_objs, dec_objs_grained, c_layout, uc_layout, data_b0)
        ...
        # elif self.type_ == 'v2_full':
            # mu, logvar, orig_gt_boxes, orig_gt_angles, orig_gt_shapes, orig_boxes, orig_angles, boxes, angles, obj_and_shape, keep = self.vae_v3.forward(
    ...
    def load_networks(self, exp, epoch, strict=True, restart_optim=False):
        if self.type_ == 'v1_box':
            ...
            for parameters in self.vae_v2.parameters():
                parameters.requires_grad = False
            print(colored('[*] freeze original vae', 'blue'))
    ...
    def sample_box_and_shape(self, dec_objs, dec_triplets, dec_sdfs, encoded_dec_text_feat, encoded_dec_rel_feat, dec_boxes, dec_angles, attributes=None, gen_shape=False):
        # if self.type_ == 'v1_full':
        #     return self.vae.sample_3dfront(point_classes_idx, self.mean_est, self.cov_est, dec_objs, dec_triplets, attributes)
        # elif self.type_ == 'v2_full':
        #     return self.vae_v2.sample(self.mean_est, self.cov_est, dec_objs, dec_triplets, dec_sdfs, encoded_dec_text_feat, encoded_dec_rel_feat,
        ...
        # return boxes, shapes
        return self.vae_v2.sample(self.mean_est, self.cov_est, dec_objs, dec_triplets, dec_sdfs, encoded_dec_text_feat, encoded_dec_rel_feat, dec_boxes, dec_angles,
                                   attributes, gen_shape=gen_shape)
    ...
    def save(self, exp, outf, epoch, counter=None):
        ...
        torch.save(self.vae_v3.state_dict(epoch, counter), os.path.join(exp, outf, 'modelcc{}.pth'.format(epoch)))
