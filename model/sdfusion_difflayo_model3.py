...
from networks.vqvae_networks.network import VQVAE
from networks.diffusion_networks.network import DiffusionUNet, MyDiffusionNet
from model_utils import load_vqvae
...
# ldm util
# from model.networks.diffusion_networks.ldm_diffusion_util 
from networks.diffusion_networks.ldm_diffusion_util import (
    make_beta_schedule,
    extract_into_tensor,
    noise_like,
    exists,
...
from networks.diffusion_networks.samplers.ddim import DDIMSampler
...
class Diffusion4Layout(BaseModel):
    def __init__(self, opt):
        super(Diffusion4Layout, self).__init__()
        ...
        shape_res = ddconfig.resolution # 64
        z_ch, n_down = ddconfig.z_channels, len(ddconfig.ch_mult) - 1
        z_sp_dim = shape_res // (2 ** n_down)
        self.z_shape = (z_ch, z_sp_dim, z_sp_dim, z_sp_dim) # [3, 16, 16, 16]

        df_model_params = df_conf.model.params
        unet_params = df_conf.unet.params
        self.df = MyDiffusionNet() # unet_params, vq_conf=vq_conf, conditioning_key=df_model_params.conditioning_key
    ...
    def forward(self):
        ...
        t = torch.randint(0, self.num_timesteps, (self.x.shape[0],), device=self.device).long()
        z_noisy, target, loss, loss_dict = self.p_losses(self.x, c_rel, t) # [B,3,16,16,16], [B, 1, 1280], [B]
