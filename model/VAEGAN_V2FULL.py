...
class Sg2ScVAEModel(nn.Module):
    ...
    def __init__(self, vocab, diff_opt, diffusion_bs=8, embedding_dim=128, batch_size=32,
        ...
        self.obj_embeddings_ec = nn.Embedding(num_objs + 1, obj_embedding_dim) # embedding_dim: 64
        self.pred_embeddings_ec = nn.Embedding(num_preds, embedding_dim * 2) # 128
        self.obj_embeddings_dc = nn.Embedding(num_objs + 1, obj_embedding_dim) # 64
        self.pred_embeddings_dc = nn.Embedding(num_preds, embedding_dim) # 64
        if self.decoder_cat:
            self.pred_embeddings_dc = nn.Embedding(num_preds, embedding_dim * 2) # 128
            self.pred_embeddings_man_dc = nn.Embedding(num_preds, embedding_dim * 3) # 192
        self.d3_embeddings = nn.Linear(num_box_params, box_embedding_dim) # 6->48
        if self.use_angles:
            self.angle_embeddings = nn.Embedding(Nangle, angle_embedding_dim) # 24
    ...
    def encoder(self, objs, triples, boxes_gt, attributes, enc_text_feat, enc_rel_feat, angles_gt=None):
        O, T = objs.size(0), triples.size(0) # num_objs, num_rels
        s, p, o = triples.chunk(3, dim=1)  # All have shape (T, 1)
        s, p, o = [x.squeeze(1) for x in [s, p, o]]  # Now have shape (T,)
        edges = torch.stack([s, o], dim=1)  # Shape is (T, 2)

        obj_vecs = self.obj_embeddings_ec(objs) # o_i
        pred_vecs = self.pred_embeddings_ec(p) # r_ij
        d3_vecs = self.d3_embeddings(boxes_gt) # b_i //6params
        if self.clip:
            obj_vecs_ = torch.cat([enc_text_feat, obj_vecs], dim=1) # p_i & o_i
            pred_vecs_ = torch.cat([enc_rel_feat, pred_vecs], dim=1) # p_ij & r_ij
        else:
            obj_vecs_, pred_vecs_ = obj_vecs, pred_vecs
        if self.use_angles:
            angle_vecs = self.angle_embeddings(angles_gt) # b_i //angle
            obj_vecs_ = torch.cat([obj_vecs_, d3_vecs, angle_vecs], dim=1)
        else:
            obj_vecs_ = torch.cat([obj_vecs_, d3_vecs], dim=1)
        
        # Box-Enhanced Contextual Graph done...
        # Graph Encoding start...
        obj_vecs_, pred_vecs_ = self.gconv_net_ec_box(obj_vecs_, pred_vecs_, edges) # Ec
    ...
    def encoder_2(self, z, objs, triples, dec_text_feat, dec_rel_feat, attributes, manipulate=False):
        ...
        rel_vecs_ = torch.cat([obj_vecs_, z], dim=1) # [num_objs, 640]
        rel_vecs_2 = None
        if self.use_E2:
    ...
    def select_sdfs(self, dec_objs_to_scene, dec_objs, dec_objs_grained, dec_sdfs, uc_rel_feat, c_rel_feat, random=False):
        ...
        for i in range(batch_size):
            sdf_candidates = dec_sdfs[np.where(dec_objs_to_scene == i)[0]] # [7, 1, 64, 64, 64]
            obj_cat = dec_objs[np.where(dec_objs_to_scene == i)[0]] # [7]
            obj_cat_grained = dec_objs_grained[np.where(dec_objs_to_scene == i)[0]] # [7]
            uc_rel = uc_rel_feat[np.where(dec_objs_to_scene == i)[0]] # [7, 1, 1280]
            c_rel = c_rel_feat[np.where(dec_objs_to_scene == i)[0]] # [7, 1, 1280]
    ...
    def forward(self, enc_objs, enc_triples, enc, enc_text_feat, enc_rel_feat, attributes, enc_objs_to_scene, dec_objs, dec_objs_grained,
        ...
        for i in range(len(missing_nodes)):
          ...
          zeros.requires_grad = True
          zeros = zeros.float().cuda()
          z = torch.cat([z[:ad_id], zeros, z[ad_id:]], dim=0) # [117, 64]

        # mark changes in nodes
        change_repr = []
        for i in range(len(z)):
            if i not in nodes_added and i not in manipulated_nodes:
                noisechange = np.zeros(self.embedding_dim) # unchanged
            else:
                noisechange = np.random.normal(0, 1, self.embedding_dim) # changed
            change_repr.append(torch.from_numpy(noisechange).float().cuda())
        change_repr = torch.stack(change_repr, dim=0)
        z_prime = torch.cat([z, change_repr], dim=1) # [117, 128]
        z_prime = self.manipulate(z_prime, dec_objs, dec_triples, dec_text_feat, dec_rel_feat, attributes) # [117, 64]
        ...
        uc_rel_feat, c_rel_feat = self.encoder_2(z, dec_objs, dec_triples, dec_text_feat, dec_rel_feat, attributes) # [117, 1, 1280]
        if c_rel_feat == None:
        ...
        if self.use_angles:
            dec_man_enc_pred, angles_pred = self.decoder(z, dec_objs, dec_triples, dec_text_feat, dec_rel_feat, attributes) # layout decoder
            orig_angles = []
            orig_gt_angles = []
        else:
