...
from helpers_util import fit_shapes_to_box, params_to_8points, params_to_8points_no_rot, params_to_8points_3dfront, get_textured_objects_v2, get_sdfusion_models, get_bbox, get_generated_models_v1, get_generated_models_v2, trimeshes_to_pytorch3d, normalize_py3d_meshes
import json
import torch
import cv2
import pyrender
...
from diff_utils.util import tensor2im
from diff_utils.util_3d import render_sdf, render_meshes, sdf_to_mesh
...
def render_v1_full(model_type, scene_id, cats, predBoxes, predAngles=None, datasize='small', classes=None, classed_idx=None, shapes_pred=None, render_type='txt2shape',
    ...
    bath_path = '/home/weiy1/code/commonscenes/experiments/g2sv1/all/renders/' + model_dict[model_type]
    if datasize == 'small':
        bath_path += '_small'
    if no_stool:
        bath_path += '_no_stool'
...
def render_v2_box(model_type, scene_id, cats, predBoxes, predAngles=None, datasize='small', classes=None,
    ...
    # bath_path = '/media/ymxlzgy/Data/graphto3d_v2_test/'+model_dict[model_type]
    # bath_path = '/mnt/hdd1/3DSSG_dataset/results_qual/' + model_dict[model_type] + epoch
    bath_path = '/home/weiy1/code/commonscenes/experiments/g2bv2/renders/' + model_dict[model_type]
    ...
    if not os.path.exists(mesh_dir):
            os.makedirs(mesh_dir)
    if render_type == 'retrieval':
        lamp_mesh_list, trimesh_meshes, raw_meshes = get_textured_objects_v2(box_and_angle, datasize, cats, classes, mesh_dir, render_boxes=render_boxes,
    ...
    if mani == 2:
        ...
        # trimesh_meshes = all_meshes
    if store_img and not demo:
        img_path = os.path.join(bath_path, "render_imgs")
        if not os.path.exists(img_path):
            os.makedirs(img_path)
        color_img = render_img(trimesh_meshes)
        color_bgr = cv2.cvtColor(color_img, cv2.COLOR_RGBA2BGR)
...
def render_v2_full(model_type, scene_id, cats, predBoxes, predAngles=None, datasize='small', classes=None, classed_idx=None, shapes_pred=None, render_type='txt2shape',
    ...
    # bath_path = '/mnt/hdd1/3DSSG_dataset/results_qual/' + model_dict[model_type] + epoch
    bath_path = '/home/weiy1/code/commonscenes/experiments/g2sv2/all_demo/renders/' + model_dict[model_type] + epoch
    # bath_path = '/home/weiy1/code/commonscenes/experiments/g2sv2/all_oursv1_wo_llm_w_iou/renders/' + model_dict[model_type] + epoch
    if datasize == 'small':
        bath_path += '_small'
    if no_stool:
        bath_path += '_no_stool'
